...
export function EmployeeDirectory({ employees }: EmployeeDirectoryProps) {
  ...
  const statuses = Array.from(
    new Set(employees.map((employee) => employee.employment_status).filter(Boolean))
  );
            ...
            {filteredEmployees.length === 0 ? (
              ...
              filteredEmployees.map((employee) => (
                <tr key={employee.id} className="hover:bg-slate-50">
                  ...
                  <Cell href={`/employees/${employee.id}`} strong>
                    {employee.full_name}
                  </Cell>
                  <Cell href={`/employees/${employee.id}`}>
                    {employee.ic_number || "-"}
...
function Cell({
  ...
  return (
    <td
      className={`px-4 py-3 ${
        strong ? "font-medium text-slate-950" : "text-slate-600"
      }`}
    >
